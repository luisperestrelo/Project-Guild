...
**Micro Rules:** `EvaluateMicroRules(runner, node)` runs when the assignment hits a Work step. Returns a gatherable index (GatherHere), -1 (FinishTask), -2 (NoMatch = stuck), or -3 (NoEligibleGatherable = all gatherables above runner's skill level). `ResolveGatherHereIndex` handles GatherAny (IntParam=-1): filters the node's gatherables by `MinLevel` at selection time — "any" means "any the runner is capable of." If no eligible gatherables exist, returns `MicroResultNoEligibleGatherable` (-3) which triggers `GatheringFailed` with `NotEnoughSkill`. `ReevaluateMicroDuringGathering` runs after each item gathered — may switch resource, FinishTask, or stop with NoMicroRuleMatched. "Let it break" philosophy: if no rule matches, the runner stays idle and a warning event fires.

**`HasAnyEligibleGatherable(runnerId, nodeId)`** — utility method on `GameSimulation`. Checks if at least one gatherable at a node is within the runner's skill level. Currently uncalled — available for future UI use (e.g., greying out node-click Work At when no gatherables are accessible).
...
- **Micro rules (MicroRulesetLibrary):** Per-Work-step, not per-runner. What happens during the Work step (GatherHere, FinishTask). Default: `Always → GatherAny` (IntParam=-1, picks random eligible resource). Evaluated at Work step entry and re-evaluated after each item gathered. `GatherHere` supports item-ID resolution via `StringParam` (e.g., `"iron_ore"` resolves to gatherable index at current node) alongside positional `IntParam`. GatherAny filters by `MinLevel` at selection time — only gatherables the runner is skilled enough for are considered.
...
On `Awake()`, creates the `GameSimulation` with config from the asset. On `StartNewGame()`, creates the world map from the asset (or fallback) and passes it to the simulation. Exposes `WorldMapAsset` property so the view layer (GameBootstrapper) can extract per-node prefab data without coupling to the data layer's SO structure.
...
**Skill XP progress bars (Live Stats):** Tracks per-skill XP changes using `_lastKnownSkillXp` and `_skillXpLastChangeTime` arrays. When XP changes, the timestamp is updated. Bars are shown for skills where `Time.time - lastChangeTime < LiveStatsXpDisplayWindowSeconds`, sorted by most recent. Pool of bar elements (label + ProgressBar) is created once, shown/hidden as needed. Reset on runner switch via `ResetSkillXpTracking()`. Driven by `UIManager` SerializeField values: `LiveStatsMaxSkillXpBars` (max simultaneous bars) and `LiveStatsXpDisplayWindowSeconds` (fade window).
