...
namespace ProjectGuild.Tests
{
    ...
    public class AutomationLibraryTests
    {
        ...
        [Test]
        public void LoadState_MigratesLegacyMacroRuleset()
        {
            // Construct an old-format game state with legacy fields
            ...
            // Create a runner with legacy macro ruleset (no ID fields)
            ...
            runner.MacroRuleset = new Ruleset();
            runner.MacroRuleset.Rules.Add(new Rule
            {
                Label = "Legacy rule",
                Conditions = { Condition.Always() },
                Action = AutomationAction.WorkAt("mine"),
            ...
            oldState.Runners.Add(runner);

            // Load the old state
            var sim = new GameSimulation(_config, tickRate: 10f);
            sim.LoadState(oldState);

            // Verify migration
            Assert.IsNotNull(runner.MacroRulesetId, "MacroRulesetId should be set after migration");
            var migratedRuleset = sim.FindMacroRulesetInLibrary(runner.MacroRulesetId);
            Assert.IsNotNull(migratedRuleset, "Migrated ruleset should be in library");
            Assert.AreEqual(1, migratedRuleset.Rules.Count, "Migrated ruleset should have the rule");
        }

        [Test]
        public void LoadState_MigratesLegacyMicroRuleset_ToWorkStep()
        {
            // Construct old-format state with legacy micro ruleset
            var oldState = new GameState();
            var map = new WorldMap();
            map.HubNodeId = "hub";
            map.AddNode("hub", "Hub");
            ...
            var runner = new Runner
            {
                ...
                Name = "Legacy Runner",
                CurrentNodeId = "mine",
                State = RunnerState.Gathering,
            };
            runner.MicroRuleset = new Ruleset();
            runner.MicroRuleset.Rules.Add(new Rule
            {
                Label = "Legacy gather",
                Conditions = { Condition.Always() },
                Action = AutomationAction.GatherHere(0),
            ...
            // Give the runner a task sequence with a Work step (no MicroRulesetId)
            runner.TaskSequence = new TaskSequence
            {
                Id = "legacy-seq",
                ...
                {
                    new TaskStep(TaskStepType.TravelTo, "mine"),
                    new TaskStep(TaskStepType.Work), // no MicroRulesetId
            ...
            // Verify migration: micro ruleset moved to library, Work step gets MicroRulesetId
            Assert.IsNull(runner.MicroRuleset, "Legacy MicroRuleset should be cleared after migration");

            var seq = sim.FindTaskSequenceInLibrary(runner.TaskSequenceId);
            Assert.IsNotNull(seq, "Task sequence should be in library");
            Assert.IsNotNull(seq.Steps[1].MicroRulesetId,
                "Work step should have MicroRulesetId after migration");

            var migratedMicro = sim.FindMicroRulesetInLibrary(seq.Steps[1].MicroRulesetId);
            Assert.IsNotNull(migratedMicro, "Migrated micro ruleset should be in library");
            Assert.AreEqual(1, migratedMicro.Rules.Count, "Should have the legacy rule");
        }
    }
}
